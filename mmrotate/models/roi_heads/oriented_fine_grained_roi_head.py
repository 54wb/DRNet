...
from mmrotate.core import rbbox2roi, convert_label
from ..builder import ROTATED_HEADS
from .rotate_standard_roi_head import RotatedStandardRoIHead


@ROTATED_HEADS.register_module()
class OrientedFineGrainedRoIHead(RotatedStandardRoIHead):
    ...
    def forward_train(self,
        ...
        #fine_grained cls and loss, the loss only have cls loss
        if self.with_cls:
            cls_results = self._cls_forward_train(sampling_results, gt_bboxes, gt_labels,
                                                  bbox_results['bbox_feats'], img_metas)
            losses.update(cls_results['loss_fine_cls'])
    ...
    def _cls_forward_train(self, sampling_results, gt_bboxes, gt_labels, bbox_feats, img_metas):
        """Run forward function and calculate loss for fine_cls_head in training.
        Args:
            x: list of multi-level img features
            sampling_results: list of sampling results
            gt_bboxes (list[Tensor]): Ground truth bboxes for each image with
                shape (num_gts, 5) in [cx, cy, w, h, a] format.
            gt_labels (list[Tensor]): class indices corresponding to each box
    ...
    def _cls_forward(self, bbox_feats):
    ...
    def simple_test_bboxes(self,
        ...
        """
        assert self.with_cls, 'Fine Cls head must be implemented'
        rois = rbbox2roi(proposals)
        bbox_results = self._bbox_forward(x, rois)
        ...
        cls_results = self._cls_forward(bbox_feats)
        fine_cls_score = cls_results['fine_cls_score']


        num_proposals_per_img = tuple(len(p) for p in proposals)
        rois = rois.split(num_proposals_per_img, 0)
        cls_score = cls_score.split(num_proposals_per_img, 0)
        fine_cls_score = fine_cls_score.split(num_proposals_per_img, 0)
        ...
        for i in range(len(proposals)):
                det_bbox, det_label = self.bbox_head.get_bbox(
                    rois[i],
                    cls_score[i],
                    fine_cls_score[i],
                    bbox_pred[i],
