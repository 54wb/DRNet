...
class RotatedBBoxHead(BaseModule):
    ...
    def get_bbox(self,
        ...
        for i in range(6):
            ...
            det_scores.append(scores_)
        det_bboxes = torch.concat(det_bboxes)
        det_scores = torch.concat(det_scores)
        if cfg is None:
            return bboxes, scores_
        else:
            nmsed_bboxes, nmsed_labels = multiclass_nms_rotated(
                det_bboxes, det_scores, cfg.score_thr, cfg.nms, cfg.max_per_img)
        if nmsed_bboxes == []:
            return torch.tensor(nmsed_bboxes), torch.tensor(nmsed_labels)
        return nmsed_bboxes, nmsed_labels
